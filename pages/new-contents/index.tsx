...
import { useGetNewContents, useGetNewProducts } from "../../query/new_quries";
import { newproductsList } from "../../components/tanstackTable/columns/newproductsList";
import { newcontents } from "../../components/tanstackTable/columns/newcontents";
...
export default function NewContents() {
  ...
  const { data: contentsData } = useGetNewContents();
  const data = useMemo(() => contentsData || [], [contentsData]);
  const columns = useMemo<ColumnDef<any, any>[]>(() => newcontents, []);
                  ...
                  {row.getVisibleCells().map((cell: any) => {
                    return (
                      <TableCell key={cell.id} cell={cell}>
                        {cell.column.id === "isFree" &&
                          (cell.row.original.isFree ? "무료" : "유료")}
                        {flexRender(
